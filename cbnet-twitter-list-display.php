<?php /* widget_cbnet_plugin_twitterlist */
/**
 ...
 * @since 1.0
 */
add_action( 'admin_notices', 'cbnet_twitter_list_admin_notice' );

function cbnet_twitter_list_admin_notice() {
	if ( ! file_exists( CBNETTWITTERWIDGETPATH ) ) {
		echo "<div class='updated' style='background-color:#f66;'><p>" . sprintf( 'The cbnet Twitter List Display Plugin is obsolete and no longer maintained. <strong>Please <a href="%s">install the cbnet Twitter Widget Plugin</a> in order to continue to receive updates and support.</strong>', cbnet_twitter_plugin_install_path() ) . "</p></div>";
...
/**
 * widget_cbnet_plugin_twitterlist class.
...
class widget_cbnet_plugin_twitterlist extends WP_Widget {

    function widget_cbnet_plugin_twitterlist() {
        $widget_ops = array('classname' => 'widget-cbnet-plugin-twitterlist', 'description' => 'cbnet plugin widget to display Twitter list tweets' );
        $this->WP_Widget('cbnet_plugin_twitterlist', 'cbnet Twitter List', $widget_ops);
    }

    function widget( $args, $instance ) {
        extract($args);
        $title = apply_filters('widget_title', empty($instance['title']) ? 'Twitter List' : $instance['title']);
...
}).render().setList('<?php echo $twitteruserid; ?>', '<?php echo $twitteruserlist; ?>').start();
</script>
<br />
<!-- End Twitter List -->
...
<?php
    ...
    function update( $new_instance, $old_instance ) {
        ...
        $instance['listtitle'] = strip_tags($new_instance['listtitle']);
        $instance['listdesc'] = strip_tags($new_instance['listdesc']);
    ...
    function form( $instance ) {
		$defaults = array( 'title' => 'Twitter List', 'twitteruserid' => 'TwitterUserID', 'twitteruserlist' => 'Twitter List Name', 'listtitle' => 'Title', 'listdesc' => 'Description of list', 'shellbg' => '#cccccc', 'shellcolor' => '#ffffff', 'tweetbg' => '#ffffff', 'tweetcolor' => '#444444', 'tweetlink' => '#5588aa', 'scrollbar' => 'true', 'loop' => 'false', 'live' => 'true', 'hashtags' => 'true', 'timestamp' => 'true', 'avatars' => 'true', 'behavior' => 'all', 'interval' => '6000', 'rpp' => '30', 'widthauto' => 'false', 'width' => '150', 'height' => '300' );
...
<p>
<label for="<?php echo $this->get_field_id('listtitle'); ?>">List Title:</label> 
<input class="widefat" id="<?php echo $this->get_field_id('listtitle'); ?>" name="<?php echo $this->get_field_name('listtitle'); ?>" type="text" value="<?php echo $instance['listtitle']; ?>" />
</p>
<p>
<label for="<?php echo $this->get_field_id('listdesc'); ?>">Caption:</label> 
<input class="widefat" id="<?php echo $this->get_field_id('listdesc'); ?>" name="<?php echo $this->get_field_name('listdesc'); ?>" type="text" value="<?php echo $instance['listdesc']; ?>" />
</p>
<p>
